...
function OrdinaryCallBindThis(realm, F, calleeContext, thisArgument) {
  ...
  if (thisMode === "strict" || F instanceof _index.NativeFunctionValue) {
    thisValue = thisArgument;
  } else {
    // 6. Else,
    // a. If thisArgument is null or undefined, then
    if ((0, _index2.HasSomeCompatibleType)(realm, thisArgument, _index.NullValue, _index.UndefinedValue)) {
...
function OrdinaryCallEvaluateBody(realm, F, argumentsList) {
  if (F instanceof _index.NativeFunctionValue) {
    ...
    return new _completions.ReturnCompletion(G);
  } else {
    ...
    var c = realm.getRunningContext().lexicalEnvironment.evaluateAbstractCompletion(_code, F.$Strict);
    ...
    if (c instanceof _completions.JoinedAbruptCompletions) {
      ...
      throw _index.AbstractValue.createIntrospectionErrorThrowCompletion(c.joinCondition);
    } else if (c instanceof _completions.ComposedAbruptCompletion) {
      if (e !== undefined) realm.apply_effects(e);
      ...
      (0, _invariant2.default)(joinedEffects[0] instanceof _completions.ReturnCompletion);
